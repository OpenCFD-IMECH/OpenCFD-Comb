! OpenCFD-comb  Post-analysis code  -----------------------------------------
...
! LHD, Institute of Mechanics, CAS
	 ...
	 real*8::  tt
	 integer:: nx,ny,nz,m,num , N_SPEC,Istep
	 print*, "Post-processing code for OpenCFD-Comb 2.x,  output tecplot-type data"
	 print*, "Please input nx, ny, nz"
	 read(*,*) nx,ny,nz 
	 print*, "Please input N_SPEC (Number of Species"
	 read(*,*) N_SPEC
	 
      allocate(x(nx,ny,nz),y(nx,ny,nz),z(nx,ny,nz))
      allocate(d(nx,ny,nz),u(nx,ny,nz),v(nx,ny,nz),  w(nx,ny,nz),T(nx,ny,nz))
	  allocate(di(nx,ny,nz,N_SPEC))
      call read_mesh(nx,ny,nz,x,y,z)

      open(77,file="opencfd-comb.dat",form="unformatted"	)
	  read(77) Istep,tt 
      ...
      do m=1,N_SPEC 
      call read3d(77,nx,ny,nz,di(1,1,1,m))
      enddo 	  
      close(77)
	 num=1
    do while( num .ne. 0) 
      ...
      read(*,*) num 
     if(num .eq. 1) then
         call write_i(nx,ny,nz,x,y,z,d,u,v,w,T)  
     else if (num .eq. 2) then
          call write_j(nx,ny,nz,x,y,z,d,u,v,w,T)  
     else if (num .eq. 3) then
        call write_k(nx,ny,nz,x,y,z,d,u,v,w,T)  
     else if (num .eq. 4) then
        call write_3D(nx,ny,nz,x,y,z,d,u,v,w,T)     
	 endif
    enddo 
    deallocate(x,y,z,d,u,v,w,T,di)
   end
!==================================
...
!========================================
   subroutine write_i(nx,ny,nz,x,y,z,d,u,v,w,T)  
     implicit  none
     ...
     real*8,dimension(nx,ny,nz)::x,y,z,d,u,v,w,T
      ...
      write(44,*) 'variables= x,y,z,d,u,v,w,T'
      write(44,*) 'zone j=',ny, 'k=',nz
      do k=1,nz
      do j=1,ny
      write(44,'(10f16.8)') x(i,j,k),y(i,j,k),z(i,j,k),   &
              d(i,j,k),u(i,j,k),v(i,j,k),w(i,j,k), T(i,j,k)
      enddo
      enddo
     close(44)
    end
...
!c---------------------------------------------------
   subroutine write_j(nx,ny,nz,x,y,z,d,u,v,w,T)  
     implicit  none
	 integer:: nx,ny,nz,i,j,k
	 real*8:: p00
     real*8,dimension(nx,ny,nz)::x,y,z,d,u,v,w,T
...
!c---------------------------------------------------
   subroutine write_k(nx,ny,nz,x,y,z,d,u,v,w,T)  
     implicit  none
	 integer:: nx,ny,nz,i,j,k
	 real*8:: p00
     real*8,dimension(nx,ny,nz)::x,y,z,d,u,v,w,T
   ...
   subroutine write_3D(nx,ny,nz,x,y,z,d,u,v,w,T)  
     implicit  none
	 ...
	 allocate(Q(nx,ny,nz)) 
	 Q=0.d0 
	 print*, " Plot Q  ? (1 Yes,  0 No) ?"
     read(*,*) Iflag_Q 
	 if(Iflag_Q .eq. 1) then 
	 open(88,file="Q.dat",form="unformatted")
	 call read3d(88,nx,ny,nz,Q)
	 close(88)
	 endif 
	 print*, "please input ib, ie, jb, je, kb, ke, istep, jstep, kstep"
	 read(*,*) ib, ie, jb, je, kb, ke, istep, jstep, kstep 
	  open(44,file='flow3d.dat')
      write(44,*) 'variables= x,y,z,d,u,v,w,T,Q'
      write(44,*) 'zone i=', (ie-ib)/istep +1 , 'j=', (je-jb)/jstep +1, 'k=', (ke-kb)/kstep+1
	  do k=kb,ke,kstep
      do j=jb,je,jstep
      do i=ib,ie,istep 
      write(44,'(11f16.8)') x(i,j,k),y(i,j,k),z(i,j,k),   &
              d(i,j,k),u(i,j,k),v(i,j,k),w(i,j,k),  T(i,j,k),Q(i,j,k)
      enddo
      enddo
	  enddo 
     close(44)
