...
!-------------------------------------------------- 
          ...
          print*, 'tao=',tao*Lref/Uref ,'EK0=',Ek0
!-------------------------------------------------
...
!       random phase
       ...
       enddo
         ...
         do j=1,ny
         do i=1,nx
         ur(i,j,k)=ur(i,j,k)*sr*Uref         ! Uref dimensional in SI unit
         vr(i,j,k)=vr(i,j,k)*sr*Uref 
         wr(i,j,k)=wr(i,j,k)*sr*Uref 
         enddo
         enddo
         enddo
...
!     ui is density and temperature
       ...
       open(99,file="opencfd-comb0.dat",form="unformatted")
        write(99) 0, 0.d0
        ui=dref                  ! uniform density (=Dref)
 	    call write3d(99,nx,ny,nz,ui)         
       
 	    call write3d(99,nx,ny,nz,ur)
        call write3d(99,nx,ny,nz,vr)
        call write3d(99,nx,ny,nz,wr)
		ui=Tref                         ! uniform temperature 
        call write3d(99,nx,ny,nz,ui)          
		vi=dref*ci(1)         ! density of N2 
		wi=dref*ci(2)         ! density of O2 
		call write3d(99,nx,ny,nz,vi)
		call write3d(99,nx,ny,nz,wi)
       close(99)
