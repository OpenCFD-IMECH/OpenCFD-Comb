...
! code for Time averaging-----------------------------------------------
   ...
   Use flow_data     
   implicit none
   integer i,j,k,m,ierr,Kstep_save_average
   real(kind=OCFD_REAL_KIND):: ana_data(50)
   real(kind=OCFD_REAL_KIND),save,allocatable,dimension(:,:,:):: dm,um,vm,wm,Tm
   real(kind=OCFD_REAL_KIND),save,allocatable,dimension(:,:,:,:):: diav
   ...
   allocate(diav(nx,ny,nz,N_SPEC))
    ...
    if(ex) then 
	  ...
	  if(my_id .eq. 0) then  
	   ...
	   open(88,file="opencfd-comb.dat.average", form="unformatted") 
	  ...
	  do m=1,N_SPEC 
	  call read_3d(88,diav(1,1,1,m),nx,ny,nz,nx_global,ny_global,nz_global) 
      enddo	  
...
!-------------------------------------------------------
  ...
  enddo
   ...
   do j=1,ny 
   do i=1,nx 
     diav(i,j,k,m)=(K_average*diav(i,j,k,m)+di(i,j,k,m))/(K_average+1.d0)
   enddo
   enddo
   enddo
   enddo 
...
!---------------------------------------------------------
    if(mod(Istep,Kstep_save_average) .eq. 0) then
     if(my_id.eq.0) then 
     write(filename,"('OCFD-comb'I8.8'.dat.average')") Istep 
	 ...
	 do m=1,N_SPEC 
	  call write_3d(99,diav(1,1,1,m),nx,ny,nz,nx_global,ny_global,nz_global)
	 enddo 
     if(my_id .eq. 0) then 
     close(99)
...
! Ref: JCP 13(5):1415,2001
   ...
   integer i,j,k,ierr
   real(kind=OCFD_REAL_KIND):: ux,amu_aver,c_aver,d_aver,T_aver,u_rms,v_rms,w_rms, &
       VV_rms,Ux_rms,d_rms,E_kinetic,ux_skewness,ux_flatness, u_skewness, u_flatness, &
       Alamda,Re_lamda,Alamdax,Re_lamdax,Amt, atmp1(100),atmp0(100),gamma,c1
...
! Statistics
    ...
    do i=1,nx
	  ux=uk(i,j,k)*Akx(i,j,k)+ui(i,j,k)*Aix(i,j,k)+us(i,j,k)*Asx(i,j,k)
	  gamma=p(i,j,k)/Et(i,j,k)+1.d0 
	  c1=sqrt(gamma*p(i,j,k)/d(i,j,k))         ! 近似声速
      c_aver=c_aver+c1
    ...
    call MPI_ALLREDUCE(atmp1,atmp0,13,OCFD_DATA_TYPE,MPI_SUM,MPI_Comm_World,ierr)
...
!---------------------------------------------------------   
        ...
        print*, "tt, Re_lamda,Re_lamdax,Amt,vv_rms,u_rms,v_rms,w_rms," //   &
          "u_skewness,u_flatness,ux_skewness,ux_flatness," // &
          "E_kinetic,d_rms,T_aver,Amu_aver,d_aver,Ux_rms,Alamdax" 

        print*, tt, Re_lamda,Re_lamdax,Amt,vv_rms,u_rms,v_rms,w_rms,   &
          u_skewness,u_flatness,ux_skewness,ux_flatness,  &
          E_kinetic,d_rms,T_aver,Amu_aver,d_aver,Ux_rms,Alamdax 

        write(33,'(19f20.12)') tt,  Re_lamda,Re_lamdax,Amt,vv_rms,u_rms,v_rms,w_rms,   &
          u_skewness,u_flatness,ux_skewness,ux_flatness,  &
          E_kinetic,d_rms,T_aver,Amu_aver,d_aver,Ux_rms,Alamdax 
        close(33)
        endif
