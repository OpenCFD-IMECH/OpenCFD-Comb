...
! Et1: 内能；gamma=p1/Et1+1  等效比热比； c1=sqrt(gamma*p1/d1) 等效声速
  subroutine split_Stager_Warming(nx1,LAP1,Et1,p1,c1,d1,u1,v1,w1,A1,A2,A3,fP,fm)
   Use flow_para
   implicit none
   integer nx1,LAP1,i
   real(kind=OCFD_REAL_KIND),dimension (1-LAP1:nx1+LAP1):: Et1,p1,c1,d1,u1,v1,w1,A1,A2,A3
   real(kind=OCFD_REAL_KIND),dimension(1-LAP1:nx1+LAP1,5):: fp, fm 
    ...
    real(kind=OCFD_REAL_KIND):: ss,ss1,ak1,ak2,ak3,tmp0,tmp1,tmp2,tmp3,   &
          ...
          vs,uc1,uc2,vc1,vc2,wc1,wc2,vvc1,vvc2,vv,W2,P2,gamma 
      

     do i=1-LAP1,nx1+LAP1
	  gamma=p1(i)/Et1(i)+1.d0            ! 等效比热比
...
!----------------------------------------
      ...
      vv=(gamma-1.d0)*(u1(i)*u1(i)+v1(i)*v1(i) +w1(i)*w1(i))   
      W2=tmp3*c1(i)*c1(i)
...
!--------------------------------------------
 ...
 subroutine split_Local_LaxFriedrichs(nx1,LAP1,Et1,p1,c1,d1,u1,v1,w1,A1,A2,A3,fP,fm)
   Use flow_para
   implicit none
   integer nx1,LAP1,i
   real(kind=OCFD_REAL_KIND),dimension (1-LAP1:nx1+LAP1):: Et1,p1,c1,d1,u1,v1,w1,A1,A2,A3
   real(kind=OCFD_REAL_KIND),dimension(1-LAP1:nx1+LAP1,5):: fp, fm 
    ...
    real(kind=OCFD_REAL_KIND):: ss,un,lamda0,E0
     ...
     do i=1-LAP1,nx1+LAP1
	  ...
	  E0=Et1(i)+0.5d0*d1(i)*(u1(i)*u1(i)+v1(i)*v1(i)+w1(i)*w1(i))        ! Total Energy 
	  
	  fp(i,1)=0.5d0*(d1(i)*un +lamda0*d1(i))
	  fp(i,2)=0.5d0*(d1(i)*u1(i)*un+A1(i)*p1(i) + lamda0*d1(i)*u1(i))
	  fp(i,3)=0.5d0*(d1(i)*v1(i)*un+A2(i)*p1(i) + lamda0*d1(i)*v1(i))
	  fp(i,4)=0.5d0*(d1(i)*w1(i)*un+A3(i)*p1(i) + lamda0*d1(i)*w1(i))
	  fp(i,5)=0.5d0*((E0+p1(i))*un + lamda0*E0)	  

	  fm(i,1)=0.5d0*(d1(i)*un -lamda0*d1(i))
	  fm(i,2)=0.5d0*(d1(i)*u1(i)*un+A1(i)*p1(i) - lamda0*d1(i)*u1(i))
	  fm(i,3)=0.5d0*(d1(i)*v1(i)*un+A2(i)*p1(i) - lamda0*d1(i)*v1(i))
	  fm(i,4)=0.5d0*(d1(i)*w1(i)*un+A3(i)*p1(i) - lamda0*d1(i)*w1(i))
	  fm(i,5)=0.5d0*((E0+p1(i))*un - lamda0*E0)	  	  
	 enddo 
	 end 