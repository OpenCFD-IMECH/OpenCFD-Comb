...
! Characteristic flux 
  subroutine du_inviscous_Character 
	  ...
	  real(kind=OCFD_REAL_KIND):: gamma,hx_1, hy_1,hz_1,Rhb0
!c-------------------------------------------------
      Ka1=Scheme%Ka1
	  Kb1=Scheme%Kb1
	  Ka2=Scheme%Ka2
...
!c---------x direction-------------------------
   ...
   do j=1,ny  
	 ...
	 if(Para%Flux_Splitting .eq. OCFD_Split_SW) then
	  call split_Stager_Warming(nx,LAP,Et1x,p1x,c1x,d1x,u1x,v1x,w1x,A1x,A2x,A3x,fpx,fmx) 
	 else 
	  call split_Local_LaxFriedrichs(nx,LAP,Et1x,p1x,c1x,d1x,u1x,v1x,w1x,A1x,A2x,A3x,fpx,fmx) 	
     endif 	 
	 
	 call flux_charteric(nx,LAP,Ka1,Kb1,Ka2,Kb2, &
	                     hhx,fpx,fmx,A1x,A2x,A3x, &
	                     Et1x,p1x,c1x,d1x,u1x,v1x,w1x,Scheme%Bound_index(1,1),Scm_Hbx, Scheme%Scheme_boundary(1:2))
	  do m=1,5	  
	  do i=1,nx
...
!c-------y direction ---------------------------
   ...
   do i=1,nx 
    ...
    if(Scheme%Scheme_Invis == 	OCFD_Scheme_Hybrid) then 
	 ...
	 if(Para%Flux_Splitting .eq. OCFD_Split_SW) then
	  call split_Stager_Warming(ny,LAP,Et1y,p1y,c1y,d1y,u1y,v1y,w1y,A1y,A2y,A3y,fpy,fmy) 
     else 
	  call split_Local_LaxFriedrichs(ny,LAP,Et1y,p1y,c1y,d1y,u1y,v1y,w1y,A1y,A2y,A3y,fpy,fmy) 
	 endif 
	 
	 call flux_charteric(ny,LAP,Ka1,Kb1,Ka2,Kb2, &
	                     hhy,fpy,fmy,A1y,A2y,A3y,&
	                     Et1y,p1y,c1y,d1y,u1y,v1y,w1y,Scheme%Bound_index(1,2),Scm_Hby, Scheme%Scheme_boundary(3:4))
...
!c-------z direction ---------------------------
   ...
   do i=1,nx 
    ...
    if(Scheme%Scheme_Invis == 	OCFD_Scheme_Hybrid) then 
	 ...
	 if(Para%Flux_Splitting .eq. OCFD_Split_SW) then
	  call split_Stager_Warming(nz,LAP,Et1z,p1z,c1z,d1z,u1z,v1z,w1z,A1z,A2z,A3z,fpz,fmz) 
	 else 
	  call split_Local_LaxFriedrichs(nz,LAP,Et1z,p1z,c1z,d1z,u1z,v1z,w1z,A1z,A2z,A3z,fpz,fmz) 
	 endif 
	 
	  call flux_charteric(nz,LAP,Ka1,Kb1,Ka2,Kb2, &
	                hhz,fpz,fmz,A1z,A2z,A3z, &
	                Et1z,p1z,c1z,d1z,u1z,v1z,w1z,Scheme%Bound_index(1,3),Scm_Hbz, Scheme%Scheme_boundary(5:6))
...
!c-----------------------------------
	  subroutine  flux_charteric(nn,LAP1,Ka1,Kb1,Ka2,Kb2,hh,fp,fm,AA1,AA2,AA3, &
	                           EE,pp,cc1,dd,uu,vv,ww,Bound_index,Scm_Hb,Scheme_boundary)
      use flow_para
	  implicit none 
	  
	  integer nn,LAP1,Ka1,Kb1,Ka2,Kb2,i,m,mm,Bound_index(2),ia,ib,Scheme_boundary(2),Ka1h,Kb1h,Ka2h,Kb2h
      real(kind=OCFD_REAL_KIND),dimension (1-LAP1:nn+LAP1):: EE,pp,cc1,dd,uu,vv,ww,AA1,AA2,AA3
      ...
      real(kind=OCFD_REAL_KIND):: d1,u1,v1,w1,c1,p1,Et1,V2,un,ul,um,gamma
...
!-------------------------------------------------------
  ...
  do i=ia,ib
   ...
   ! Transform variables to character space 
	   ...
	   c1=(cc1(i)+cc1(i+1))*0.5d0
	   p1=(pp(i)+pp(i+1))*0.5d0 
	   Et1=(EE(i)+EE(i+1))*0.5d0 
