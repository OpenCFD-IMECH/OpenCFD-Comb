...
! Ver 1.5, 2024-12-18, 在specie.in 中使用了 Sp%SpecFlag （1 单原子， 2 双原子） 
!----------------------------------------------------------------------------
!  read_Spec( ) ;  
...
!-------------------------------------------------------------------------------
  ...
  subroutine read_Spec
    ...
    do k=1,N_Spec
      ...
      read(99,*)
      read(99,*)  Sp%name                                                     
      read(99,*)  Sp%Mi  ,   Sp%SpecFlag         ! Mi 组分摩尔质量   ;  SpecFlag 单原子为1, 双原子为2
...
! 计算定压比热 Cp   
  ...
  subroutine comput_Cv_EuckenFormula(Cv)
    ...
    integer, parameter:: MonoatomicSpecies = 1 ,DiatomicSpecies = 2 !MonoatomicSpecies单原子 ，DiatomicSpecies双原子
      Cp=0.d0
      Cv=0.d0
      do k=1,N_SPEC
        if(SPEC(k)%SpecFlag .eq. MonoatomicSpecies )then
          Cp(k) = 2.5d0 * SPEC(k)%Ri                !单原子
          Cv(k) = Cp(k) - SPEC(k)%Ri
        else
          Cp(k) = 2.5d0 * SPEC(k)%Ri + SPEC(k)%Ri   !双原子
          Cv(k) = Cp(k) - SPEC(k)%Ri
        endif
      enddo
...
!      A1=A1+(SPEC(k)%Ai(jc)-1.d0)*R1*di(k)         ! 系数，各组分加权求和 (Ai-1) for Cv
    ...
    if(T <= Tct_max ) then
	  Cp=(A1+B1*T+C1*T**2+D1*T**3+E1*T**4)/d   !这里计算得到的其实是Cv，应当是笔误写错为Cp,对后续计算其实并无影响
    else 
